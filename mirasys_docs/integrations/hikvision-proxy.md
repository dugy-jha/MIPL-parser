# Hikvision Proxy | Mirasys Help Center

Source: https://documentation.mirasys.com/integrations/integrations/hikvision-proxy

Hikvision Proxy

Hikvision Proxy is separate service which can be install to VMS server. With this service system can receive Hikvision metadata from camera. This proxy need to be install all VMS servers where you want to fetch metadata from Hikvision camera. Proxy configuration happens using VMS server Text Channels feature.
...
Now Hikvision Proxy is installed to VMS server.

After installation check that you have file under DVR-folder

UDD4HikvisionISAPI.xml

If not you can copy them from UDDConfigExamples-folder to DVR-folder

Configuration

Check that Hikvision camera is added to system using native driver and image is working
...
Proxy Service; HikvisionISAPIProxy
...
Configuration file; UDD4HikvisionISAPI.xml
...
If everything works, you should see heartBeat data on Text Channel
